...
/**
 * General game manager entity that serves as entry point for access to the model. Adherent classes
 * can be injected into the contoller. On the long run there will by multiple implementations,
 * depending on whether an in-RAM model management or Database mangament (which allows for multiple
 * instances of the service to be deployed simultaneously) is desired.
 *
...
public interface GameManager<T extends Game> {
  ...
  /**
   * Removes an indexed game. The action is rejected adn a IllegalModelAccessException is thrown, if
   * the game is not yet finished, but the corresponding flag is set to false.
   *
   ...
   * @throws ModelAccessException in case the game is still runnung by the previous parameter is set
   *                              to false
   */
